...
class BaseTrainer(object):
    def __init__(self, args):
        ...
        self.train_data = self.train_data_beatx
        if args.use_amass:
            self.train_data_amass = __import__(f"dataloaders.amass_sep_lower", fromlist=["something"]).CustomDataset(args, "train")
            beatx_len = len(self.train_data_beatx)
            smplx_len = len(self.train_data_amass)
...
if __name__ == "__main__":
    ...
    os.environ["MASTER_PORT"]=f'9{dev}75'
