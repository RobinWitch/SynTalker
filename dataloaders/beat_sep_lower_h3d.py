...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        if self.args.beat_align:
            if not os.path.exists(args.data_path+f"weights/mean_vel_smplxflame_30.npy"):
                self.calculate_mean_velocity(args.data_path+f"weights/mean_vel_smplxflame_30.npy")
            self.avg_vel = np.load(args.data_path+f"weights/mean_vel_smplxflame_30.npy")
        
        if self.args.pose_norm:
            self.mean_pose = np.load(self.args.mean_pose_path)
            self.std_pose = np.load(self.args.std_pose_path)
    ...
    def inv_transform(self, data):
        return data * self.std_pose + self.mean_pose
        ...
        for index, file_name in self.selected_file.iterrows():
            ...
            if "smplx" in self.args.pose_rep:
                ...
                if not os.path.exists(npy_path):
                    ...
                    continue
                h3d623_data = np.load(npy_path, allow_pickle=True)
                assert 30%self.args.pose_fps == 0, 'pose_fps should be an aliquot part of 30'
                stride = int(30/self.args.pose_fps)
                pose_each_file = h3d623_data[::stride] 
            ...
            if self.args.audio_rep is not None:
                ...
                audio_file = pose_file.replace("smplxflame_30", 'wave16k').replace(ext, ".wav")
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            if self.args.pose_norm:
                tar_pose = (tar_pose - self.mean_pose) / self.std_pose
                tmr_tar_pose=(tar_pose - self.tmr_mean_pose) / self.tmr_std_pose
                #tmr_tar_pose = tar_pose
            #print(in_shape)
            ...
            if self.loader_type == "test":
                ...
                vid = torch.from_numpy(vid).reshape((vid.shape[0], -1)).int()
                tar_pose = torch.from_numpy(tar_pose).reshape((tar_pose.shape[0], -1)).float()
                tmr_tar_pose = torch.from_numpy(tmr_tar_pose).reshape((tmr_tar_pose.shape[0], -1)).float()
                in_facial = torch.from_numpy(in_facial).reshape((in_facial.shape[0], -1)).float()
            return {"pose":tar_pose, "tmr_tar_pose":tmr_tar_pose,"audio":in_audio, "word":in_word, "id":vid,"prompt_text":"None","m_length":self.args.pose_length}
